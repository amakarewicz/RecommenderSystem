...
class ModelEvaluator:
    ...
    def __init__(self, k_list, filter_similar=False, vec_matrix=None, articles=None):
        self.k_list = k_list
    ...
    def _evaluate_model_for_user(self, model, person_id):
        ...
        if self.filter_similar == True:
            id_list = list(person_recs_df['nzz_id'])
            indices = self.articles[self.articles.nzz_id.isin(id_list)].index.tolist()

            matrix_lower = np.tril(cosine_similarity(self.vec_matrix[indices]))
