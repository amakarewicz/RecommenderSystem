...
def choose_recomm(models_recommendations: list, w_inner: list, limit: int, ratio: tuple = (1,1,1)) -> list:
...
class Recommendation(Recommendation_model):
    ...
    def recommend(self, user_id: int, limit: int = 5, 
        ...
        m_popularity = Popularity_model_final(articles_db = self.articles_db, 

                                              user_db = self.user_db)

        # m_colaborative = cf_model(articles_db = self.articles_db, 
        ...
        if self.user_db is None:
            ...
            recommended = m_popularity.recommend(user_id, limit=limit, ignored=ignored,
            ...
            for model in models:
                ...
                r, e  = model.recommend(user_id, limit=limit, ignored=ignored,
                                articles_db=self.articles_db, user_db=self.user_db, ev_return=True)
                recomm_for_each.append(r)
                evaluations.append(e)
            # choose using probability from given weights as ratio tuple, p.e. (1,2,3) 
            recommended = choose_recomm(recomm_for_each, e, limit, self.w)
        return recommended
